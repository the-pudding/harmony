...
describe("parseRomanToken — extensions", () => {
	...
	it("parses major-quality extensions", () => {
		expect(parseRomanToken("IVmaj7")).toMatchObject({
			degree: 4,
			suffix: "maj7"
		});
		expect(parseRomanToken("Imaj9")).toMatchObject({
			degree: 1,
			suffix: "maj9"
		});
		expect(parseRomanToken("V7")).toMatchObject({ degree: 5, suffix: "7" });
		expect(parseRomanToken("IV9")).toMatchObject({ degree: 4, suffix: "9" });
		expect(parseRomanToken("Isus4")).toMatchObject({
			degree: 1,
			suffix: "sus4"
		});
		expect(parseRomanToken("Isus2")).toMatchObject({
			degree: 1,
			suffix: "sus2"
		});
		expect(parseRomanToken("I(add9)")).toMatchObject({
			degree: 1,
			suffix: "add9"
		});
		expect(parseRomanToken("IV(add6)")).toMatchObject({
			degree: 4,
			suffix: "6"
		});
	});
	...
	it("parses minor-quality extensions", () => {
		expect(parseRomanToken("ii7")).toMatchObject({
			degree: 2,
			suffix: "minor7"
		});
		expect(parseRomanToken("iii7")).toMatchObject({
			degree: 3,
			suffix: "minor7"
		});
		expect(parseRomanToken("vi7")).toMatchObject({
			degree: 6,
			suffix: "minor7"
		});
		expect(parseRomanToken("vi9")).toMatchObject({
			degree: 6,
			suffix: "minor9"
		});
		expect(parseRomanToken("iim7")).toMatchObject({
			degree: 2,
			suffix: "minor7"
		});
	});
	...
	it("strips slash annotations and returns the root chord", () => {
		expect(parseRomanToken("IV/V")).toMatchObject({
			degree: 4,
			suffix: "major"
		});
		expect(parseRomanToken("I/3")).toMatchObject({
			degree: 1,
			suffix: "major"
		});
		expect(parseRomanToken("IVmaj7/3")).toMatchObject({
			degree: 4,
			suffix: "maj7"
		});
		expect(parseRomanToken("vi/V/3")).toMatchObject({
			degree: 6,
			suffix: "minor"
		});
	});
	...
	it("strips outer parentheses (borrowed chords)", () => {
		...
		expect(parseRomanToken("(ii)")).toMatchObject({
			degree: 2,
			suffix: "minor"
		});
	});
	...
	it("romanTokensToParsedProgression produces correct pitch classes in C major", () => {
		...
		expect(parsed!.map((c) => c.suffix)).toEqual([
			"maj7",
			"minor7",
			"minor7",
			"major"
...
describe("romanTokensToParsedProgression — scale-aware parsing (burnin up regression)", () => {
	...
	it("minor and major parsings of i-III-VII-VI produce different pitch classes", () => {
		const minor = romanTokensToParsedProgression(
			["i", "III", "VII", "VI"],
			"minor"
		)!;
		const major = romanTokensToParsedProgression(
			["i", "III", "VII", "VI"],
			"major"
		)!;
		expect(minor.map((c) => c.rootPitchClass)).not.toEqual(
			major.map((c) => c.rootPitchClass)
		);
	});

	it("defaults to major scale when scale is omitted", () => {
		const withDefault = romanTokensToParsedProgression(["I", "V", "vi", "IV"]);
		const withMajor = romanTokensToParsedProgression(
			["I", "V", "vi", "IV"],
			"major"
		);
