...
export const embeddingMethodDescriptions: Record<EmbeddingMethod, MethodDescription> =
	{
		umap: {
			title: embeddingMethodLabels.umap,
			summary:
				"UMAP over TF-IDF vectors of matched named progressions (exact progression identity).",
			rationale:
			...
			approach:
				"Each song's matched progression counts are TF-IDF-weighted and L2-normalized. UMAP builds a fuzzy nearest-neighbor graph under cosine distance, then optimizes a 2D layout that preserves that local structure. A fixed random seed keeps the layout stable between runs.",
			tradeoffs:
		...
		content: {
			...
			rationale:
				"Progression and Chord grams treat each progression as a bag of raw chord transitions, so vi-ii-V-I and ii-V-I share almost nothing. This method expands each matched progression into content features shared across related progressions, then weights by match count and chorus emphasis — so it inherits UMAP's noise filtering but gains harmonic relatedness. A larger neighbor count and SVD pre-reduction give it better global structure than the Progression method.",
			approach:
